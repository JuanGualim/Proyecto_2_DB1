...
export const getReporteVentas = async () => {
  const [rows] = await pool.query("SELECT * FROM vista_reporte_ventas");
  return rows;
};
  ...
  const [rows] = await pool.query(`
    ...
    HAVING COUNT(v.id_venta) > (
      SELECT AVG(total_compras) FROM (
        SELECT COUNT(id_venta) AS total_compras FROM venta GROUP BY id_cliente
      ) AS sub
    )
  `);
  return rows;
...
export const getClientesElite = async () => {
  const [rows] = await pool.query(`
    ...
    WHERE total_gastado > (SELECT AVG(total_gastado) FROM total_por_cliente)
  `);
  return rows;
};
